...
class FlowNetBase(ModelDesc):
    def __init__(self, height=None, width=None, name = 'Flownet'):
        self.height = height
        self.width = width
        self.name = name
    ...
    def build_graph(self, left, right, reuse = False):
    ...
    @property
    def vars_train(self):
        return [var for var in tf.global_variables() if ((self.name in var.name) and ('flownet_fusion' in var.name))]
...
class FlowNet2(FlowNetBase):
    ...
    def flownet2_fusion(self, x):
        ...
        '''
        conv0 = tf.layers.conv2d(pad(x, 1), 64, name='conv0', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')

        x = tf.layers.conv2d(pad(conv0, 1), 64, name='conv1', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv1 = tf.layers.conv2d(pad(x, 1), 128, name='conv1_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        x = tf.layers.conv2d(pad(conv1, 1), 128, name='conv2', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv2 = tf.layers.conv2d(pad(x, 1), 128, name='conv2_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        ...
        flow2 = tf.layers.conv2d(pad(conv2, 1), 2, name='predict_flow2', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow2_up = tf.layers.conv2d_transpose(flow2, 2, name='upsampled_flow2_to_1', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(conv2, 32, name='deconv1', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow1 = tf.layers.conv2d(pad(interconv1, 1), 2, name='predict_flow1', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow1_up = tf.layers.conv2d_transpose(flow1, 2, name='upsampled_flow1_to_0', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(concat1, 16, name='deconv0', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
    ...
    def flownet2_sd(self, x):
        ...
        with argscope([tf.layers.conv2d], activation=lambda x: tf.nn.leaky_relu(x, 0.1),
            ...
            argscope([tf.layers.conv2d_transpose], padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4):
        '''
        x = tf.layers.conv2d(pad(x, 1), 64, name='conv0', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')

        x = tf.layers.conv2d(pad(x, 1), 64, name='conv1', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv1 = tf.layers.conv2d(pad(x, 1), 128, name='conv1_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
        ...
        x = tf.layers.conv2d(pad(conv2, 1), 256, name='conv3', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv3 = tf.layers.conv2d(pad(x, 1), 256, name='conv3_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        x = tf.layers.conv2d(pad(conv3, 1), 512, name='conv4', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv4 = tf.layers.conv2d(pad(x, 1), 512, name='conv4_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        x = tf.layers.conv2d(pad(conv4, 1), 512, name='conv5', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv5 = tf.layers.conv2d(pad(x, 1), 512, name='conv5_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        x = tf.layers.conv2d(pad(conv5, 1), 1024, name='conv6', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', strides=2, kernel_size=3,
                      data_format='channels_first')
        conv6 = tf.layers.conv2d(pad(x, 1), 1024, name='conv6_1', strides=1, activation=lambda x: tf.nn.leaky_relu(x, 0.1),
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        ...
        flow6 = tf.layers.conv2d(pad(conv6, 1), 2, name='predict_flow6', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow6_up = tf.layers.conv2d_transpose(flow6, 2, name='upsampled_flow6_to_5', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(conv6, 512, name='deconv5', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow5 = tf.layers.conv2d(pad(interconv5, 1), 2, name='predict_flow5', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow5_up = tf.layers.conv2d_transpose(flow5, 2, name='upsampled_flow5_to_4', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(concat5, 256, name='deconv4', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow4 = tf.layers.conv2d(pad(interconv4, 1), 2, name='predict_flow4', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow4_up = tf.layers.conv2d_transpose(flow4, 2, name='upsampled_flow4_to_3', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(concat4, 128, name='deconv3', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow3 = tf.layers.conv2d(pad(interconv3, 1), 2, name='predict_flow3', strides=1, activation=tf.identity,
                      padding='valid', kernel_size=3,
                      data_format='channels_first')
        flow3_up = tf.layers.conv2d_transpose(flow3, 2, name='upsampled_flow3_to_2', padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4)
        x = tf.layers.conv2d_transpose(concat3, 64, name='deconv2', activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='same',
                     data_format='channels_first', strides=2, kernel_size=4)
    ...
    def graph_structure(self, x, standalone=True):
        ...
        with argscope([tf.layers.conv2d], activation=lambda x: tf.nn.leaky_relu(x, 0.1),
            ...
            argscope([tf.layers.conv2d_transpose], padding='same', activation=tf.identity,
                     data_format='channels_first', strides=2, kernel_size=4):
        '''
        x = tf.layers.conv2d(pad(x, 3), 64, kernel_size=7, name='conv1', activation=lambda x: tf.nn.leaky_relu(x, 0.1),
        ...
        flow6 = tf.layers.conv2d(pad(conv6, 1), 2, name='predict_flow6', strides=1, activation=tf.identity,
                                 padding='valid', kernel_size=3,
                                 data_format='channels_first')
        flow6_up = tf.layers.conv2d_transpose(flow6, 2, name='upsampled_flow6_to_5', use_bias=False, padding='same',
        ...
        flow5 = tf.layers.conv2d(pad(concat5, 1), 2, name='predict_flow5', strides=1, activation=tf.identity,
                                 padding='valid', kernel_size=3,
                                 data_format='channels_first')
        flow5_up = tf.layers.conv2d_transpose(flow5, 2, name='upsampled_flow5_to_4', use_bias=False, padding='same',
                                              activation=tf.identity,
                                              data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow4 = tf.layers.conv2d(pad(concat4, 1), 2, name='predict_flow4', strides=1, activation=tf.identity,
                                 padding='valid', kernel_size=3,
                                 data_format='channels_first')
        flow4_up = tf.layers.conv2d_transpose(flow4, 2, name='upsampled_flow4_to_3', use_bias=False, padding='same',
                                              activation=tf.identity,
                                              data_format='channels_first', strides=2, kernel_size=4)
        ...
        flow3 = tf.layers.conv2d(pad(concat3, 1), 2, name='predict_flow3', strides=1, activation=tf.identity,
                                 padding='valid', kernel_size=3,
                                 data_format='channels_first')
        flow3_up = tf.layers.conv2d_transpose(flow3, 2, name='upsampled_flow3_to_2', use_bias=False, padding='same',
                                              activation=tf.identity,
                                              data_format='channels_first', strides=2, kernel_size=4)
...
class FlowNet2C(FlowNetBase):
    def graph_structure(self, x1x2):
        ...
        x = tf.layers.conv2d(pad(x1x2, 3), 64, kernel_size=7, name='conv1',
                             activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid', strides=2,
                             data_format='channels_first')
        conv2 = tf.layers.conv2d(pad(x, 2), 128, kernel_size=5, name='conv2',
                                 activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid', strides=2,
                                 data_format='channels_first')
        ...
        conv_redir = tf.layers.conv2d(conv3a, 32, kernel_size=1, strides=1, name='conv_redir',
                                 activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid',
                                 data_format='channels_first')
        ...
        conv3_1 = tf.layers.conv2d(pad(in_conv3_1, 1), 256, name='conv3_1', strides=1,
                                 activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid',kernel_size=3,
                                 data_format='channels_first')

        x = tf.layers.conv2d(pad(conv3_1, 1), 512, name='conv4',
                                 activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid', strides=2,kernel_size=3,
                                 data_format='channels_first')
        conv4 = tf.layers.conv2d(pad(x, 1), 512, name='conv4_1', strides=1,
                                 activation=lambda x: tf.nn.leaky_relu(x, 0.1), padding='valid', kernel_size=3,
                                 data_format='channels_first')
