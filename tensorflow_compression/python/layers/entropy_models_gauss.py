...
class EntropyBottleneck_gauss(base_layer.Layer):
  """Entropy bottleneck layer.
  ...
  def __init__(self, init_scale=10, filters=(3, 3, 3), tail_mass=1e-9,
    ...
    super(EntropyBottleneck_gauss, self).__init__(**kwargs)
  ...
  def build(self, input_shape):
    ...
    self.n = input_shape[0].value
    self.h = input_shape[1].value
    self.w = input_shape[2].value
    self.c = input_shape[3].value
    if channels is None:
  ...
  def build_gauss(self, stddev):
      ...
      tail_prob = constant_op.constant(1/(65536.0*100), dtype=self.dtype, shape=(self.n, self.h, self.w, self.c, 1))
      median_prob = constant_op.constant(0.5, dtype=self.dtype, shape=(self.n, self.h, self.w, self.c, 1))
      head_prob = constant_op.constant(1 - 1/(65536.0*100), dtype=self.dtype, shape=(self.n, self.h, self.w, self.c, 1))
      ...
      tail_quantile = norm_dist.quantile(tail_prob)
      median_quantile = norm_dist.quantile(median_prob)
      head_quantile = norm_dist.quantile(head_prob)

      # Save medians for `call`, `compress`, and `decompress`.
      self._medians = median_quantile
      ...
      minima = math_ops.reduce_max(self._medians - tail_quantile)
      maxima = math_ops.reduce_max(head_quantile - self._medians)
      minmax = math_ops.maximum(minima, maxima)
      minmax = math_ops.ceil(minmax)
      minmax = math_ops.maximum(minmax, 1)
      minmax = math_ops.minimum(minmax, 1e3)
      ...
      pmf = abs(norm_dist.cdf(samples + half) - norm_dist.cdf(samples - half))
      ...
      pmf = array_ops.concat([
        math_ops.add_n([pmf[:, :, :, :, :1], norm_dist.cdf(samples[:, :, :, :, :1] - half)]),
        pmf[:, :, :, :, 1:-1],
        math_ops.add_n([pmf[:, :, :, :, -1:], norm_dist.cdf(-(samples[:, :, :, :, -1:] + half))]),
      ], axis=-1)
      self._pmf = pmf

      self._quantized_cdf = coder_ops.pmf_to_quantized_cdf(
        pmf, precision=self.range_coder_precision)
      self._quantized_cdf = array_ops.squeeze(self._quantized_cdf, [0])
  ...
  def call(self, inputs, input_stddev, training):
    """Pass a tensor through the bottleneck.
    ...
    Raises:
      ValueError: if `inputs` has different `dtype` or number of channels than
        a previous set of inputs the model was invoked with earlier.
    """
    values, likelihood = self.call_gauss(inputs, input_stddev, training)
